...
class RecyclerCampeonatosCreados:
    RecyclerView.Adapter<RecyclerCampeonatosCreados.CampeonatoViewHolder>() {
        ...
        inner class CampeonatoViewHolder(val binding: EnmarcadoCampeonatoBinding):
            RecyclerView.ViewHolder(binding.root) {
            fun binding(campeonato: Campeonato) {
                binding.botonEntrarAlCampeonato.setOnClickListener{
                    val intentCampeonato = Intent(context, ActivityDentroDelCampeonato::class.java)
                    intentCampeonato.putExtra("ID_CAMPEONATO", campeonato.id.toString())
                    context?.startActivity(intentCampeonato)
                }
